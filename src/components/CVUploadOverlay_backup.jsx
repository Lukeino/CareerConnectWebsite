...
const CVUploadOverlay = ({ isOpen, onClose, onUpload, onDeleteCV, currentCV, userId }) => {
  ...
  // Utilizza la configurazione API centralizzata per coerenza  const getStaticFileUrl = (filename) => {
    ...
    // SOLUZIONE SEMPLICE: Usa la stessa logica delle altre API calls
    // In produzione '/api' viene automaticamente reindirizzato da Netlify al backend EC2
    const fullUrl = `${API_CONFIG.BASE_URL.replace('/api', '')}/uploads/${filename}`;
